...
void prompt_subtraction(){
  ...
  for(int it_y=0; it_y<year.size(); it_y++){
    ...
    for(int it_mc=0; it_mc<MCNumber; it_mc++){
      ...
      for(int j=1; j<=20; j++){
        this_content_loose = h_MC_LooseMuPtCone[it_mc][it_y]->GetBinContent(j)*0.6798656507922819; // TODO this can be changed with each ID.
    ...
    c1->SaveAs("./plots_Subt/FakeRate_PtCone_"+year.at(it_y)+".png");
    ...
    for(int it_mc=0; it_mc<MCNumber; it_mc++){
      ...
      for(int i=1; i<=2; i++){
        for(int j=1; j<=5; j++){
          this_content_loose = h_MC_LooseMu[it_mc][it_y]->GetBinContent(i,j)*0.6798656507922819;
          this_content_tight = h_MC_TightMu[it_mc][it_y]->GetBinContent(i,j)*0.6798656507922819;
      ...
      for(int i=3; i<=4; i++){
        for(int j=1; j<=5; j++){
          this_content_loose = h_MC_LooseMu[it_mc][it_y]->GetBinContent(i,j)*1.261913344446241;
