...
struct StudyConfig {
  TString subdir;
  vector<ComparisonEntry> entries;

  StudyConfig() : subdir(""), entries() {}
};

struct PanelConfig {
  bool draw_ratio_panel;

  PanelConfig(bool draw_ratio_panel_in=false)
      : draw_ratio_panel(draw_ratio_panel_in) {}
};

struct PlotConfig {
  NominalConfig nominal;
  StudyConfig study;
  PanelConfig panel;
};
...
NominalConfig BuildNominalConfig(TString WP_nom, bool DrawExt)
{
  ...
  if (DrawExt) {
    ...
    cfg.wp_name = WP_nom + "_Ext";
    cfg.ext_nominal_index = (cfg.wp_noms.size() > 1) ? 1 : -1;
  }
  else {
    cfg.wp_noms = {WP_nom};
    cfg.wp_name = WP_nom;
    cfg.ext_nominal_index = -1;
  }

  cfg.n_nominal_limits = cfg.wp_noms.size();
  return cfg;
}

StudyConfig BuildStudyConfig(TString WP_nom, int SepLimit, TString method)
{
  StudyConfig cfg;
  ...
  if(SepLimit==1){
    ...
    cfg.subdir = "SRSplit";
    cfg.entries = {
      ComparisonEntry(WP_nom, "_HNL_sr1_syst_Combined", "", kRed, 0.01, method),
      ComparisonEntry(WP_nom, "_HNL_sr2_syst_Combined", "", kBlue, 0.01, method),
      ComparisonEntry(WP_nom, "_HNL_sr3_syst_Combined", "", kViolet, 0.01, method)
    };
  }
...
PanelConfig BuildPanelConfig(bool CompareLimits)
{
...
TString BuildDefaultWPDescription(TString wp)
{
  TString tmp = wp;

  if (tmp.BeginsWith("ANv")) {
    Ssiz_t first_underscore = tmp.Index("_");
    if (first_underscore != kNPOS) tmp.Remove(0, first_underscore + 1);
  }
  ...
  tmp.ReplaceAll("_Preapproval", "");
  tmp.ReplaceAll("_Decorr_JetDecorr", "");
  tmp.ReplaceAll("_", " ");
...
TString BuildDefaultTagDescription(TString tag)
{
  TString tmp = tag;

  if (tmp.BeginsWith("_")) tmp.Remove(0, 1);

  tmp.ReplaceAll("_syst_Combined", " only");
...
TString BuildComparisonDescription(const ComparisonEntry& entry)
{
  if (entry.descrp != "") return entry.descrp;
  if (entry.tag != "_HNL_syst") return BuildDefaultTagDescription(entry.tag);
...
TString MakeSafeDirName(TString name)
{
  ...
  out.ReplaceAll(",", "_");
  out.ReplaceAll("+", "Plus");
  out.ReplaceAll("-", "Minus");
  while (out.Contains("__")) out.ReplaceAll("__", "_");

  return out;
}

TString BuildComparisonSubdirName(TString requested_name, const vector<ComparisonEntry>& entries)
{
  if (requested_name != "") return MakeSafeDirName(requested_name);
  if (entries.empty()) return "";

  TString auto_name = "compare";
  const size_t n_entries_for_name = std::min<size_t>(entries.size(), 3);
  for (size_t i = 0; i < n_entries_for_name; ++i) {
    auto_name += "_" + BuildComparisonDescription(entries[i]);
  }
...
void DrawLimits(TString year="", TString channel="", bool DrawExt=false, bool AddPub=true, int SepLimit=0, bool CompareLimits=false, bool AppendLimitTable=false, bool IsXsecLimit=false, bool Logy=true){
  ...
  if(DrawExt && SepLimit){
    cout << "[INFO] DrawExt and SepLimit are not supported simultaneously. Sorry!" << endl;
    cout << "[INFO] Exiting..." << endl;
    return;
  }
  if(AddPub && SepLimit){
  ...
  const size_t n_nominal_limits = plot_cfg.nominal.n_nominal_limits;
  const int ext_nominal_index = plot_cfg.nominal.ext_nominal_index; // M500 cut-based nominal in DrawExt mode
  ...
  vector<TString> compare_descrps;
  for (const auto &entry : compare_entries) {
    compare_descrps.push_back(BuildComparisonDescription(entry));
  }

  TString base_plotpath = plotpath + WP_name;
  TString this_plotpath = base_plotpath;
  if(!compare_entries.empty() && !DrawExt){
    TString compare_dir = BuildComparisonSubdirName(compare_subdir, compare_entries);
    if(compare_dir != "") this_plotpath += "/" + compare_dir;
  }

  if(gSystem->AccessPathName(this_plotpath)){
  ...
  for(int i=0; i<files.size(); i++){
    ...
    while(getline(in,elline)){
      ...
      if(IsXsecLimit){
        ...
        if(this_mass==500.){
          this_SSWWxsec = 0;
          if(i==ext_nominal_index) this_SSWWxsec = GetSSWWxsec(500, channel); // DrawExt: Ext M500 uses SSWW, BDT M500 does not.
        }
        if(channel=="EMu"){
  ...
  if (!DrawExt && !compare_entries.empty()){
    ...
    for (size_t entry_idx = 0; entry_idx < compare_entries.size(); ++entry_idx) {
      ...
      if (limit_idx >= limits.size()) {
        cout << "[WARNING] Missing limit payload for comparison entry " << entry_idx << ". Skipping..." << endl;
        continue;
      }

      std::set<double> set0(masses[0].begin(), masses[0].end());
      std::set<double> seti(masses[limit_idx].begin(), masses[limit_idx].end());
      ...
      if (common_mass_list.empty()) {
        std::cout << "[WARN] No common masses between nominal and comparison <"
                  << compare_descrps[entry_idx] << ">. Skipping upper-panel overlay.\n";
        continue;
      }

      vector<double> y_limit;
      ...
      if (y_limit.size() != common_mass_list.size()) {
        cout << "[WARNING] Comparison <" << compare_descrps[entry_idx]
             << "> is incomplete. Skipping upper-panel overlay..." << endl;
        continue;
      }
...
//=======================================
  ...
  if(draw_ratio_panel) dummy->GetXaxis()->SetLabelSize(0);
  ...
  if(draw_ratio_panel){
    ...
    for (size_t entry_idx = 0; entry_idx < compare_entries.size(); ++entry_idx) {
      const size_t limit_idx = n_nominal_limits + entry_idx;
      if (limit_idx >= limits.size()) {
        cout << "[WARNING] Missing limit payload for ratio entry " << entry_idx << ". Skipping..." << endl;
        continue;
      }
      ...
      for (size_t j = 0; j < this_mass.size(); ++j) {
        ...
        if (it != mass_nominal.end()) {
          ...
          cout << "[INFO] Mass " << m << " in comparison <" << compare_descrps[entry_idx]
               << "> not found in nominal. Skipping..." << endl;
        }
      }

      if (mass_i.empty()) {
        cout << "[WARN] No common masses between nominal and comparison <"
             << compare_descrps[entry_idx] << ">. Skipping lower-panel ratio." << endl;
        continue;
      }

      ratio_vs_nominal.push_back(ratio_i);
      mass_vs_nominal.push_back(mass_i);
      ratio_descrps.push_back(compare_descrps[entry_idx]);
    ...
    for (size_t i = 0; i < ratio_vs_nominal.size(); ++i) {
      ...
      gr->SetLineWidth(2);
      gr->SetLineColor(ratio_colors[i]);
      gr->SetMarkerColor(ratio_colors[i]);
