...
def make_frame_hist(name, nbins, labels, ymin, ymax, title):
    """
    Create a uniquely-named, directory-detached frame histogram to avoid:
      TROOT::Append: Replacing existing TH1: frame (Potential memory leak).
    """
    hframe = ROOT.TH1F(name, title, nbins, 0.5, nbins + 0.5)
    hframe.SetDirectory(0)  # detach from current gDirectory to avoid name collisions
    hframe.GetYaxis().SetRangeUser(ymin, ymax)
    hframe.GetYaxis().SetTitle("PDF ratio to nominal")
...
def graph_band_from_points(xs, ymins, ymaxs, fill_color, alpha=0.35, line_color=None):
    ...
    for i, (x, ylo, yhi) in enumerate(zip(xs, ymins, ymaxs)):
        if ylo is None or yhi is None:
            # keep a placeholder point with zero error so we maintain index alignment
            g.SetPoint(i, x, 0.0)
...
def draw_edge_lines(xs, ymins, ymaxs, color, style=1, width=3):
    g_lo = ROOT.TGraph()
    g_hi = ROOT.TGraph()
    g_lo.SetName("")  # avoid registering with default "Graph"
    g_hi.SetName("")
    ilo = ihi = 0
...
def draw_one_plot(base_dir, era, flavour, masses, sr, outdir, print_all_keys=False):
    ...
    # Weinberg band (if present as first bin in SR2/SR3 layout)
    g_wb = graph_band_from_points(xs, wb_lo, wb_hi, ROOT.kGray+1, alpha=0.25)
    have_wb = has_any_pair(wb_lo, wb_hi)

    # draw wider band first so narrower outline is visible
    bands_keep = []
    if have_wb:
        g_wb.Draw("E2 same")
    ...
    # optional separator if Weinberg exists as the first bin
    ...
    ensure_dir(outdir)
    base = f"{era}_{flavour}_SR-{sr}_PDFBands"
    full_base_band = os.path.join(outdir, "pdf_bands_plots_" + base)
    for ext in ("png", "pdf"):
        c1.SaveAs(f"{full_base_band}.{ext}")
    c1.Close()

    # ---------- Per-bin lines (omitted saving in your original; keeping same) ----------
    ...
    # If you want to save these too, uncomment:
    # full_base_lines = os.path.join(outdir, "pdf_bands_plots_" + f"{era}_{flavour}_SR-{sr}_Lines")
    # for ext in ("png", "pdf"):
...
def main():
    ap = argparse.ArgumentParser(description="Plot PDF variation bands vs mass using ROOT.")
    ...
    ap.add_argument("--outdir", default="pdf_bands_plots", help="Output directory for plots")
